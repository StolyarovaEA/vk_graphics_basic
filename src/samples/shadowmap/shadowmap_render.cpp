...
void SimpleShadowmapRender::AllocateResources()
{
  ...
  rsmPosition = m_context->createImage(etna::Image::CreateInfo{
    .extent     = vk::Extent3D{ 2048, 2048, 1 },
    .name       = "rsm_positon",
    .format     = vk::Format::eR32G32B32A32Sfloat,
    .imageUsage = vk::ImageUsageFlagBits::eColorAttachment | vk::ImageUsageFlagBits::eSampled });

  rsmNormal = m_context->createImage(etna::Image::CreateInfo{
    .extent     = vk::Extent3D{ 2048, 2048, 1 },
    .name       = "rsm_normal",
    .format     = vk::Format::eR32G32B32A32Sfloat,
    .imageUsage = vk::ImageUsageFlagBits::eColorAttachment | vk::ImageUsageFlagBits::eSampled });
  ...
  rsmSamplesVec.reserve(128);
  int h = 0;
  for (size_t i = 0; i < 128; i++)
    rsmSamplesVec.push_back({ halton(h++, 2) * 2 - 1, halton(h++, 2) * 2 - 1 });
...
void SimpleShadowmapRender::loadShaders()
{
  ...
  etna::create_program("simple_rsm",
    { VK_GRAPHICS_BASIC_ROOT "/resources/shaders/simple.vert.spv", VK_GRAPHICS_BASIC_ROOT "/resources/shaders/deffered.frag.spv" });
}
...
void SimpleShadowmapRender::SetupSimplePipeline()
{
  ...
  m_pBindings->BindImage(0, rsmDepth.getView({}), defaultSampler.get(), VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER);
  ...
  m_rsmPipeline          = pipelineManager.createGraphicsPipeline("simple_rsm",
             { .vertexShaderInput = sceneVertexInputDesc,
               .blendingConfig    = {
                    .attachments = { blendAttachment, blendAttachment, blendAttachment } },
               .fragmentShaderOutput = { .colorAttachmentFormats = { vk::Format::eR32G32B32A32Sfloat, vk::Format::eR32G32B32A32Sfloat, vk::Format::eR8G8B8A8Srgb }, .depthAttachmentFormat = vk::Format::eD16Unorm } });
}
...
void SimpleShadowmapRender::BuildCommandBufferSimple(VkCommandBuffer a_cmdBuff, VkImage a_targetImage, VkImageView a_targetImageView)
{
  ...
  //
  {
    etna::RenderTargetState renderTargets(a_cmdBuff, { 2048, 2048 }, {
                                                                       { rsmPosition.get(), rsmPosition.getView({}) },
                                                                       { rsmNormal.get(), rsmNormal.getView({}) },
                                                                       { rsmFlux.get(), rsmFlux.getView({}) },
                                                                     },
      rsmDepth);
  ...
  //
  {
    ...
    auto set = etna::create_descriptor_set(simpleMaterialInfo.getDescriptorLayoutId(0), a_cmdBuff, {
                                                                                                     ...
                                                                                                     etna::Binding{ 1, rsmDepth.genBinding(defaultSampler.get(), vk::ImageLayout::eShaderReadOnlyOptimal) },
                                                                                                     etna::Binding{ 2, rsmPosition.genBinding(defaultSampler.get(), vk::ImageLayout::eShaderReadOnlyOptimal) },
                                                                                                     etna::Binding{ 3, rsmNormal.genBinding(defaultSampler.get(), vk::ImageLayout::eShaderReadOnlyOptimal) },
                                                                                                     etna::Binding{ 4, rsmFlux.genBinding(defaultSampler.get(), vk::ImageLayout::eShaderReadOnlyOptimal) },
                                                                                                     etna::Binding{ 5, rsmSamples.genBinding() },
                                                                                                   });

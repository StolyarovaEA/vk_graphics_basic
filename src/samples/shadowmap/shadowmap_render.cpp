...
void SimpleShadowmapRender::PreparePipelines()
{
  ...
  m_pFSQuadLightDepth->Create(m_context->getDevice(),
    ...
    vk_utils::RenderTargetInfo2D{ VkExtent2D{ m_width, m_height }, m_swapchain.GetFormat(),                                        // this is debug full scree quad
      ...
      VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL });// seems we need LOAD_OP_LOAD if we want to draw quad to part of screen
  m_pFSQuadGNormal->Create(m_context->getDevice(),
    VK_GRAPHICS_BASIC_ROOT "/resources/shaders/quad.vert.spv",
    VK_GRAPHICS_BASIC_ROOT "/resources/shaders/quad.frag.spv",
    vk_utils::RenderTargetInfo2D{ VkExtent2D{ m_width, m_height }, m_swapchain.GetFormat(), VK_ATTACHMENT_LOAD_OP_LOAD, VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL, VK_IMAGE_LAYOUT_PRESENT_SRC_KHR });
  SetupSimplePipeline();
}
...
void SimpleShadowmapRender::loadShaders()
{
  etna::create_program("simple_material",
    { VK_GRAPHICS_BASIC_ROOT "/resources/shaders/quad.vert.spv", VK_GRAPHICS_BASIC_ROOT "/resources/shaders/simple_shadow.frag.spv" });
...
void SimpleShadowmapRender::BuildCommandBufferSimple(VkCommandBuffer a_cmdBuff, VkImage a_targetImage, VkImageView a_targetImageView)
{
...

  {
    std::array barriers
      ...
      VkImageMemoryBarrier2{ .sType = VK_STRUCTURE_TYPE_IMAGE_MEMORY_BARRIER_2, .srcStageMask = VK_PIPELINE_STAGE_COLOR_ATTACHMENT_OUTPUT_BIT, .srcAccessMask = VK_ACCESS_COLOR_ATTACHMENT_READ_BIT, .dstStageMask = VK_PIPELINE_STAGE_COLOR_ATTACHMENT_OUTPUT_BIT, .dstAccessMask = VK_ACCESS_COLOR_ATTACHMENT_WRITE_BIT, .oldLayout = VK_IMAGE_LAYOUT_UNDEFINED, .newLayout = VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL, .srcQueueFamilyIndex = VK_QUEUE_FAMILY_IGNORED, .dstQueueFamilyIndex = VK_QUEUE_FAMILY_IGNORED, .image = gNormalMap.get(), .subresourceRange = {
  ...
  {
    float scaleAndOffsetLightDepth[] = { 0.25f, 0.25f, -0.75f, 0.75f };
    m_pFSQuadLightDepth->SetRenderTarget(a_targetImageView);
    m_pFSQuadLightDepth->DrawCmd(a_cmdBuff, m_quadDSLightDepth, scaleAndOffsetLightDepth);
    float scaleAndOffsetGNormal[] = { 0.25f, 0.25f, -0.25f, 0.75f };
    m_pFSQuadGNormal->SetRenderTarget(a_targetImageView);
    m_pFSQuadGNormal->DrawCmd(a_cmdBuff, m_quadDSGNormal, scaleAndOffsetGNormal);
  }

...
void SimpleShadowmapRender::AllocateResources()
{
  ...
  shadowMap = m_context->createImage(etna::Image::CreateInfo
  {
   ...
   preProcessedImage = m_context->createImage(etna::Image::CreateInfo{
    .extent     = vk::Extent3D{ m_width, m_height, 1 },
    .format     = vk::Format::eB8G8R8A8Unorm,
    .imageUsage = vk::ImageUsageFlagBits::eColorAttachment | vk::ImageUsageFlagBits::eSampled | vk::ImageUsageFlagBits::eStorage });

  processedImage = m_context->createImage(etna::Image::CreateInfo{
    .extent     = vk::Extent3D{ m_width, m_height, 1 },
    .format     = vk::Format::eB8G8R8A8Unorm,
...
void SimpleShadowmapRender::SetupSimplePipeline()
{
  ...
  m_gaussianPipeline     = pipelineManager.createComputePipeline("simple_compute", {});
}
...
void SimpleShadowmapRender::BuildCommandBufferSimple(VkCommandBuffer a_cmdBuff, VkImage a_targetImage, VkImageView a_targetImageView)
{
  ...
  //
  {
    etna::RenderTargetState renderTargets(a_cmdBuff, {2048, 2048}, {}, shadowMap.getView({}));
    {
      ...
      auto set              = etna::create_descriptor_set(simpleShadowInfo.getDescriptorLayoutId(0), { etna::Binding{ 0, vk::DescriptorBufferInfo{ constants.get(), 0, VK_WHOLE_SIZE } } });
  ...
  //
  {
    ...
    etna::RenderTargetState renderTargets(a_cmdBuff, { m_width, m_height }, { { preProcessedImage.getView({}) } }, mainViewDepth.getView({}));
  ...
  if(m_input.drawFSQuad)
  {
    ...
    m_pFSQuad->SetRenderTarget(preProcessedImage.getView({}));
  ...
  
  {
    std::array barriers
      ...
      VkImageMemoryBarrier2{ .sType = VK_STRUCTURE_TYPE_IMAGE_MEMORY_BARRIER_2, .srcStageMask = VK_PIPELINE_STAGE_COLOR_ATTACHMENT_OUTPUT_BIT, .srcAccessMask = VK_ACCESS_COLOR_ATTACHMENT_WRITE_BIT, .dstStageMask = VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT, .dstAccessMask = VK_ACCESS_SHADER_WRITE_BIT, .oldLayout = VK_IMAGE_LAYOUT_UNDEFINED, .newLayout = VK_IMAGE_LAYOUT_GENERAL, .srcQueueFamilyIndex = VK_QUEUE_FAMILY_IGNORED, .dstQueueFamilyIndex = VK_QUEUE_FAMILY_IGNORED, .image = processedImage.get(), .subresourceRange = {
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        .aspectMask     = VK_IMAGE_ASPECT_COLOR_BIT,
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        .baseMipLevel   = 0,
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        .levelCount     = 1,
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        .baseArrayLayer = 0,
  ...
  // Gaussian blur
  {
    ...
    auto set               = etna::create_descriptor_set(simpleComputeInfo.getDescriptorLayoutId(0), {
                                                                                                       etna::Binding{ 0, vk::DescriptorImageInfo{ defaultSampler.get(), preProcessedImage.getView({}), vk::ImageLayout::eGeneral } },
                                                                                                       etna::Binding{ 1, vk::DescriptorImageInfo{ defaultSampler.get(), processedImage.getView({}), vk::ImageLayout::eGeneral } },
                                                                                                       etna::Binding{ 2, vk::DescriptorBufferInfo{ gaussianCoeffs.get(), 0, VK_WHOLE_SIZE } },
                                                                                       });

    VkDescriptorSet vkSet = set.getVkSet();

    vkCmdBindPipeline(a_cmdBuff, VK_PIPELINE_BIND_POINT_COMPUTE, m_gaussianPipeline.getVkPipeline());
...

  {
    std::array barriers{
      ...
      VkImageMemoryBarrier2{ .sType = VK_STRUCTURE_TYPE_IMAGE_MEMORY_BARRIER_2, .srcStageMask = VK_PIPELINE_STAGE_COLOR_ATTACHMENT_OUTPUT_BIT, .srcAccessMask = VK_ACCESS_COLOR_ATTACHMENT_WRITE_BIT, .dstStageMask = VK_PIPELINE_STAGE_TRANSFER_BIT, .dstAccessMask = VK_ACCESS_TRANSFER_WRITE_BIT, .oldLayout = VK_IMAGE_LAYOUT_UNDEFINED, .newLayout = VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, .srcQueueFamilyIndex = VK_QUEUE_FAMILY_IGNORED, .dstQueueFamilyIndex = VK_QUEUE_FAMILY_IGNORED, .image = a_targetImage, .subresourceRange = {
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          .aspectMask     = VK_IMAGE_ASPECT_COLOR_BIT,
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          .baseMipLevel   = 0,
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          .levelCount     = 1,
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          .baseArrayLayer = 0,
  ...
  vkCmdBlitImage(a_cmdBuff, processedImage.get(), VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL, a_targetImage, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, 1, &imageBlitRegion, VK_FILTER_NEAREST);

...
layout (binding = 2) uniform sampler2D vsmShadow;
...
void main()
{
  ...
  if(Params.vsm)
  {
    vec2 vsm = textureLod(vsmShadow, shadowTexCoord, 0).xy;
    float expected = vsm.x;
    float diff = posLightSpaceNDC.z - expected;
    float variance = vsm.y - expected*expected;
    float val = variance/(variance + diff*diff);
    shadow  = ((posLightSpaceNDC.z < expected + 0.001f) || outOfView) ? 1.0f : val;
  }
  else
